...
namespace Helper
{
    class ShanuDGVHelper
    {
        ...
        static String ImageName = @"N:\Images\toggle.png";
        String FilterColumnName1 = "";
        String FilterColumnName2 = "";
        ...
        public void DGVMasterGridClickEvents(DataGridView ShanuMasterDGV, DataGridView ShanuDetailDGV, int columnIndexs, ShanuEventTypes eventtype, ShanuControlTypes types,DataTable DetailTable,String FilterColumn1)
        {
            MasterDGVs = ShanuMasterDGV;
            DetailDGVs = ShanuDetailDGV;
            gridColumnIndex = columnIndexs;
            DetailgridDT = DetailTable;
            FilterColumnName1 = FilterColumn1;
           // FilterColumnName2 = FilterColumn2;
        ...
        private void masterDGVs_CellContentClick_Event(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                MasterDGVs.Rows[e.RowIndex].Cells[0].Value = Image.FromFile(@"N:\Images\expand.png");

                if (e.ColumnIndex == gridColumnIndex)
                {


                    if (ImageName == @"N:\Images\expand.png")
                    {
                        DetailDGVs.Visible = true;
                        ImageName = @"N:\Images\toggle.png";
                        // cols.Image = Image.FromFile(ImageName);
                        MasterDGVs.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Image.FromFile(ImageName);


                        String Filterexpression1 = MasterDGVs.Rows[e.RowIndex].Cells[FilterColumnName1].Value.ToString();
                        //String Filterexpression2 = MasterDGVs.Rows[e.RowIndex].Cells[FilterColumnName2].Value.ToString();
                        MasterDGVs.Controls.Add(DetailDGVs);

                        Rectangle dgvRectangle = MasterDGVs.GetCellDisplayRectangle(1, e.RowIndex, true);
                        DetailDGVs.Size = new Size(MasterDGVs.Width - 100, 200);
                        DetailDGVs.Location = new Point(dgvRectangle.X, dgvRectangle.Y + 20);


                        DataView detailView = new DataView(DetailgridDT);
                        detailView.RowFilter = FilterColumnName1 + " = '" + Filterexpression1 + "'";
